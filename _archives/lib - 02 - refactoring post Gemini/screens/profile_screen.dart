...
// V004 - Refactoring : Suppression du paramètre deviceId. L'écran s'appuie entièrement sur l'UID Firebase (FirebaseAuth.currentUser) pour charger et sauvegarder le profil dans users/{uid}.
//      - Suppression de la logique obsolète basée sur devices/{deviceId}.
//      - Suppression du passage de deviceId lors de la navigation vers HomeSelector. - 2025/05/29
// V003 - ajout import cloud_firestore pour FirebaseFirestore & SetOptions - 2025/05/24 10h31 (Historique hérité de LoginScreen / RegisterScreen)
// V002 - ajout import firebase_auth pour get uid/email & import firestore_service pour saveUserProfile - 2025/05/25 (Dates ajustées)
...
import '../services/firestore_service.dart'; // Pour saveUserProfile et getUserProfile (maintenant basés sur UID)
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  // Stocke le prénom initial chargé pour comparer dans _onDisplayNameChanged
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      final userData = await getUserProfile(user.uid); // Utilise firestore_service refactorisé
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      await saveUserProfile( // Utilise firestore_service refactorisé
        uid: user.uid, // UID de l'utilisateur actuel
        email: user.email ?? '', // Email de Firebase Auth
        firstName: newName, // Le nouveau prénom
      );
