...
// =============================================================
// 👤 UTILISATEUR — Gestion du profil utilisateur
// =============================================================

// =============================================================
// ⚠️ ATTENTION : Duplication volontaire de l’adresse email
// =============================================================
// Ce service sauvegarde l'email dans Firestore (dans users/{uid}),
// en plus de FirebaseAuth, pour simplifier l’UI et centraliser les données.
// ➕ Avantage : accès immédiat à l’email avec le profil utilisateur.
// ➖ Inconvénient : nécessite une logique de synchronisation future.
//
// 🛠️ TODO GEM V008 : Lors d’un updateEmail() dans FirebaseAuth,
// ajouter une mise à jour explicite de Firestore (users/{uid}).
// =============================================================
...
// 📄 SUITE de lib/services/firestore_service.dart
  ...
  Future<Recipient?> getRecipient({required String userId, required String recipientId}) async {
    ...
    try {
      ...
      debugLog(
        '❌ [FirestoreService - getRecipient] Erreur inattendue lors du chargement destinataire $recipientId pour l\'UID $userId : $e', // Correction de la chaîne de log ici
        level: 'ERROR',
      );
      rethrow;
    }
...
// =============================================================
  // Ajout: Méthode pour mettre à jour les données d'un destinataire spécifique (pour l'utilisateur appelant)
  // et synchroniser les champs pertinents chez l'autre utilisateur (vue miroir).
  // Utile pour sauvegarder les modifications faites dans EditRecipientScreen (nom, icône, relation, etc.).
  // Utilise un WriteBatch pour assurer l'atomicité des deux écritures.
  Future<void> updateRecipient({required String userId, required Recipient recipient}) async {
    debugLog("📝🔄 [FirestoreService - updateRecipient] Tentative de mise à jour bidirectionnelle du destinataire ${recipient.id} pour l'UID : $userId", level: 'INFO');
    if (userId.isEmpty || recipient.id.isEmpty) {
      debugLog("⚠️ [FirestoreService - updateRecipient] UID utilisateur ou destinataire vide. Mise à jour annulée.", level: 'WARN');
      throw ArgumentError("Invalid user or recipient ID for update."); // Lancer une erreur
    }
    try {
      ...
      // Prépare les données à mettre à jour dans le document de l'utilisateur appelant (inclut tous les champs du Recipient)
      // Ceci permet de sauvegarder les champs unilatéraux (comme des notes) en plus des champs synchronisés.
      Map<String, dynamic> userUpdates = recipient.toMap();

      // Prépare les données à mettre à jour dans le document miroir chez le destinataire (uniquement les champs synchronisés)
      Map<String, dynamic> mirrorUpdates = {
        'displayName': recipient.displayName, // Nom vu par l'utilisateur appelant
        'icon': recipient.icon, // Icône vue par l'utilisateur appelant
        'relation': recipient.relation, // Relation vue par l'utilisateur appelant
        // Ajouter les autres champs synchronisés ici si nécessaire (ex: allowedPacks si c'est une liste partagée)
      };


      // 1. Mettre à jour le document destinataire chez l'utilisateur actuel (userId)
      DocumentReference userRecipientDocRef = _firestore
          .collection('users')
          .doc(userId) // UID de l'utilisateur actuel
          .collection('recipients')
          .doc(recipient.id); // UID du destinataire

      // Utilise set avec merge: true pour ne pas écraser d'éventuels autres champs spécifiques à cette vue
      // et pour créer le document si exceptionnellement il manquait (moins probable après pairUsers).
      batch.set(userRecipientDocRef, userUpdates, SetOptions(merge: true));

      // 2. Mettre à jour le document miroir chez le destinataire (recipient.id)
      DocumentReference otherUserRecipientDocRef = _firestore
          .collection('users').doc(recipient.id) // UID du destinataire
          .collection('recipients').doc(userId); // UID de l'utilisateur actuel dans sa liste

      // Utilise set avec merge: true pour ne pas écraser d'éventuels autres champs spécifiques à la vue de l'autre utilisateur
      // et pour créer le document si exceptionnellement il manquait.
      batch.set(otherUserRecipientDocRef, mirrorUpdates, SetOptions(merge: true));

      // Exécute le batch d'écritures de manière atomique
      await batch.commit();

      debugLog("✅ [FirestoreService - updateRecipient] Destinataire ${recipient.id} mis à jour bidirectionnellement avec succès pour l'UID $userId.", level: 'SUCCESS');
      ...
      // Gérer l'erreur 'not-found' si le document destinataire n'existe pas (ex: supprimé par l'autre utilisateur)
      if (e.code == 'not-found') {
        debugLog("⚠️ [FirestoreService - updateRecipient] Document destinataire ${recipient.id} non trouvé pour mise à jour bidirectionnelle pour l'UID $userId. Ou document miroir manquant chez ${recipient.id}.", level: 'WARN');
        // Peut-être lancer une erreur spécifique ou retourner false si l'appelant doit savoir que le document n'existe plus.
      }
      rethrow; // Rethrow l'exception
    } catch (e) { // Gère toute autre erreur inattendue
...
// 📄 SUITE de lib/services/firestore_service.dart
    ...
    try {
      ...
      if (e.code == 'not-found') {
        debugLog("⚠️ [FirestoreService - deleteRecipient] Document destinataire $recipientId non trouvé pour suppression pour l'UID $userId.", level: 'WARN'); // Correction de la chaîne de log ici
        // Peut-être ignorer cette erreur ou la logger différemment.
      }
      rethrow; // Rethrow l'exception
    } catch (e) { // Gère toute autre erreur inattendue
      debugLog(
        '❌ [FirestoreService - deleteRecipient] Erreur inattendue lors de la suppression destinataire $recipientId pour l\'UID $userId : $e', // Correction de la chaîne de log ici
        level: 'ERROR',
      );
      rethrow;
    }
  ...
  Future<void> sendMessage({required String senderUid, required String recipientUid, required Message message}) async {
    ...
    try {
      ...
      final data = message.toMap(); // Utilise toMap() du modèle Message (qui inclus from/to/content/sentAt)
      ...
      // dans les documents Recipient des deux utilisateurs pour faciliter l'affichage de la liste des conversations.
...
} // <-- Fin de la classe FirestoreService
